...
it also advertises the two cloud-only tools the gateway implements — `search_libraries`
(find cloud libraries in the catalog, including ones not linked yet) and `link_library`
(link one so `search` / `explore` / `list` can use it) — and the server instructions
